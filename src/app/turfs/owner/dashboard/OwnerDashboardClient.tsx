...
export default function OwnerDashboardClient({
    ...
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        if (turfsData.success) {
          const normalized: OwnerTurf[] = turfsData.data.map((item: OwnerTurf) => ({
            ...item,
            name: item.title,
  ...
  async function handleUpdate(id: string): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/ownerTurfs/${id}`,
        {
          method: "PATCH",
          headers: {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Logged in as:{" "}
            <span className="text-emerald-600 font-semibold">{userEmail}</span>
          </p>
        </div>
        <Link
          href="/turfs/add"
      ...
      {isDataLoading ? (
                  ...
                  {turfs.map((turf) => {
                    ...
                    return (
                      <tr key={turf._id} className="hover:bg-slate-50/50">
                          ...
                          {isEditing ? (
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                ...
                                className="border p-1.5 rounded-lg text-xs w-full max-w-45"
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 text-xs font-bold border rounded-full uppercase ${getStatusStyle(turf.status)}`}
                          >
                            {turf.status || "pending"}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          {isEditing ? (
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                onClick={() =>
                                  handleDelete(turf._id, turf.name)
                                }
                                className="px-3 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold rounded-lg"
                              >
                                Delete
                              </button>
                            </div>
        ...
        <div className="text-center py-16 border border-dashed border-slate-200 rounded-3xl bg-white">
          ...
          <Link
            href="/turfs"
            className="text-emerald-600 font-bold text-sm underline mt-2 inline-block"
          >
            Browse Turfs
          </Link>
        </div>
